...
public class FeatureServiceTest : TestBase
{
    ...
    {
        var feature = await this.client.V1.Features.ArchiveFeature(
            "x",
            new(),
    ...
    {
        var feature = await this.client.V1.Features.CreateFeature(
            new()
            {
                ID = "id",
    ...
    {
        var page = await this.client.V1.Features.ListFeatures(
            new(),
            TestContext.Current.CancellationToken
        );
        page.Validate();
    ...
    {
        var feature = await this.client.V1.Features.RetrieveFeature(
            "x",
            new(),
            TestContext.Current.CancellationToken
        );
    ...
    {
        var feature = await this.client.V1.Features.UnarchiveFeature(
            "x",
            new(),
            TestContext.Current.CancellationToken
        );
    ...
    {
        var feature = await this.client.V1.Features.UpdateFeature(
            "x",
            new(),
            TestContext.Current.CancellationToken
        );
