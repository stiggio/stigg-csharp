...
public class EntitlementServiceTest : TestBase
{
    ...
    {
        var entitlement = await this.client.V1.Events.Plans.Entitlements.Create(
            "planId",
            new()
            {
                Entitlements =
    ...
    {
        var planEntitlement = await this.client.V1.Events.Plans.Entitlements.Update(
            "id",
            new() { PlanID = "planId" },
            TestContext.Current.CancellationToken
        );
    ...
    {
        var entitlements = await this.client.V1.Events.Plans.Entitlements.List(
            "planId",
            new(),
            TestContext.Current.CancellationToken
        );
    ...
    {
        var planEntitlement = await this.client.V1.Events.Plans.Entitlements.Delete(
            "id",
            new() { PlanID = "planId" },
            TestContext.Current.CancellationToken
        );
