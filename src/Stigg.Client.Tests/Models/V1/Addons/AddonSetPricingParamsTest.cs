...
public class AddonSetPricingParamsTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var parameters = new AddonSetPricingParams
        {
            ID = "x",
            PricingType = AddonSetPricingParamsPricingType.Free,
            BillingID = "billingId",
            MinimumSpend =
            [
                new()
        ...
        string expectedID = "x";
        ApiEnum<string, AddonSetPricingParamsPricingType> expectedPricingType =
            AddonSetPricingParamsPricingType.Free;
        ...
        Assert.NotNull(parameters.MinimumSpend);
        Assert.Equal(expectedMinimumSpend.Count, parameters.MinimumSpend.Count);
        for (int i = 0; i < expectedMinimumSpend.Count; i++)
        {
            Assert.Equal(expectedMinimumSpend[i], parameters.MinimumSpend[i]);
        }
        Assert.Equal(expectedOverageBillingPeriod, parameters.OverageBillingPeriod);
        Assert.NotNull(parameters.OveragePricingModels);
        Assert.Equal(expectedOveragePricingModels.Count, parameters.OveragePricingModels.Count);
        for (int i = 0; i < expectedOveragePricingModels.Count; i++)
        {
            Assert.Equal(expectedOveragePricingModels[i], parameters.OveragePricingModels[i]);
        }
        Assert.NotNull(parameters.PricingModels);
        Assert.Equal(expectedPricingModels.Count, parameters.PricingModels.Count);
        for (int i = 0; i < expectedPricingModels.Count; i++)
        {
            Assert.Equal(expectedPricingModels[i], parameters.PricingModels[i]);
        }
    }
    ...
    public void OptionalNonNullableParamsUnsetAreNotSet_Works()
    {
        ...
        Assert.Null(parameters.OverageBillingPeriod);
        Assert.False(parameters.RawBodyData.ContainsKey("overageBillingPeriod"));
        Assert.Null(parameters.OveragePricingModels);
        Assert.False(parameters.RawBodyData.ContainsKey("overagePricingModels"));
        Assert.Null(parameters.PricingModels);
        Assert.False(parameters.RawBodyData.ContainsKey("pricingModels"));
    }
    ...
    {
        AddonSetPricingParams parameters = new()
        {
            ID = "x",
            PricingType = AddonSetPricingParamsPricingType.Free,
        };
        ...
        Assert.Equal(new Uri("https://api.stigg.io/api/v1/addons/x/charges"), url);
    }
    ...
    public void CopyConstructor_Works()
    {
        ...
        AddonSetPricingParams copied = new(parameters);

        Assert.Equal(parameters, copied);
    }
}

public class AddonSetPricingParamsPricingTypeTest : TestBase
{
    [Theory]
    [InlineData(AddonSetPricingParamsPricingType.Free)]
    [InlineData(AddonSetPricingParamsPricingType.Paid)]
    [InlineData(AddonSetPricingParamsPricingType.Custom)]
    public void Validation_Works(AddonSetPricingParamsPricingType rawValue)
    {
        // force implicit conversion because Theory can't do that for us
        ApiEnum<string, AddonSetPricingParamsPricingType> value = rawValue;
        value.Validate();
    }
    ...
    {
        var value = JsonSerializer.Deserialize<ApiEnum<string, AddonSetPricingParamsPricingType>>(
    ...
    [InlineData(AddonSetPricingParamsPricingType.Paid)]
    [InlineData(AddonSetPricingParamsPricingType.Custom)]
    public void SerializationRoundtrip_Works(AddonSetPricingParamsPricingType rawValue)
    {
        // force implicit conversion because Theory can't do that for us
        ApiEnum<string, AddonSetPricingParamsPricingType> value = rawValue;
        ...
        var deserialized = JsonSerializer.Deserialize<
            ApiEnum<string, AddonSetPricingParamsPricingType>
